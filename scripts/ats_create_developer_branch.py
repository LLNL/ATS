...
def error_and_exit(code, messages):
    ...
    print(
        "Error:         C R E A T E   D E V E L O P E R   B R A N C H   A B E N D"
    )
...
def parse_arguments(args):
    parser = argparse.ArgumentParser(
        description="Script to create a developer branch for the MC project"
    )
...
if __name__ == "__main__":
    ...
    if args.feature is not None:
        if not re.match(r"^[A-Za-z0-9_-]+$", args.feature):
            print(
                "feature '%s' is invalid.  Must be alphanumeric (plus - and _)"
                % args.feature
            )
    ...
    if args.bugfix is not None:
        if not re.match(r"^[A-Za-z0-9_-]+$", args.bugfix):
            print(
                "bugfix '%s' is invalid.  Must be alphanumeric (plus - and _)"
                % args.bugfix
            )
            print("Exiting without creating branch.")
            sys.exit(-1)
    ...
    if not re.match(r"^[A-Za-z0-9_-]+$", args.user):
        print(
            "user '%s' is invalid.  Must be alphanumeric (plus - and _)"
            % args.user
        )
        print("Exiting without creating branch.")
        sys.exit(-1)
    ...
    if not os.path.isdir(".git"):
        error_and_exit(
            ...
            [
                ...
                "Please run this script from the top level of git clone",
            ],
        )
    ...
    code, out, err = runcommand_die_on_err(
    ...
    if local_branch_type != "main":
        messages = [
            "This script restricted to only operate within a clone of the 'main' branch"
        ]
        error_and_exit(-1, messages)
    ...
    if len(git_status_lines) == 1 and git_status_lines[0] == "":
        ...
        messages = [
            "Unsure of status of local branch %s. Will not continue"
            % local_branch
        ]
        messages.append("")
    ...
    if "bitbucket" in git_pull_dry_run and "From" in git_pull_dry_run:
        ...
        messages.append("")
        for line in git_pull_dry_run_lines:
            messages.append(line)
        error_and_exit(-1, messages)
    ...
    question = (
        "Do you want to push developer branch '%s' to the remote repo %s"
        % (branch_name, remote_repo)
    )
    answer = yes_or_no(question, args.yes)
    if answer == False:
        print(
            "Exiting without pushing developer branch to the repo %s"
            % branch_name
        )
        sys.exit(0)

    code, out, err = runcommand_die_on_err(
        "git push --set-upstream origin %s" % branch_name
    )
