...
class LCMachineCore(machines.Machine):
    def init(self):
        ...
        self.lastTimeSqueueCalled = (
            time.time()
        )  # note the time when 'squeue -s' command used
    ...
    def kill(self, test):
        ...
        for killTimes in range(0, 1):
            ...
            if self.lastSqueueResult is None or (
                (time.time() - self.lastTimeSqueueCalled) > 60
            ):  # in seconds
            ...
            for line in self.lastSqueueResult:
                if test.jobname in line:
                    ...
                    if debug():
                        log(
                            "---- LCMachineCore::kill: %s" % (scancelCommand),
                            echo=True,
                        )
            ...
            if debug():
                log(
                    "---- LCMachineCore::kill, CALLED AGAIN %s test name: %s %s"
                    % ((killTimes + 1), test.jobname, test.serialNumber),
                    echo=True,
                )
            time.sleep(1)
