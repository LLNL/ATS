...
class ScopeLoggers(logging.LoggerAdapter):
    ...
    def __init__(self, logger, callerContext, parms=""):
        ...
        self.debug(
            (self.__class__._indent * " ") + "=> " + self.caller + " BEGIN"
        )
    ...
    def __del__(self):
        ...
        self.__class__._indent -= 1
        self.debug(
            (self.__class__._indent * " ") + " <= " + self.caller + " END"
        )
...
def demoCallLogger(logger):
    ...
    logger.info(
        "BEFORE calling logging.debug. Debug message should appear "
        "somewhere (sent to stderr with prefix '$$$:root:')."
    )
    logging.basicConfig(level=DEBUG)
    logging.debug("Debug message from root logger")
    logger.info(
        "AFTER calling logging.debug (this message should appear only once)"
    )
...
def demoLogRecord(logger):
    ...
    class DictFilter(logging.Filter):
        def filter(self, record):
            ...
            sortedDict.sort()
            print(
                "record.__dict__ = %s" % str(sortedDict).replace("('", "\n ('")
            )
            return True
    ...
    logger.debug(
        "This log call caused the filter to put out the dict above.",
        extra=dict(context="BAR"),
    )
...
def demoLoggerAdapter(logger):
    ...
    class PLogger(logging.LoggerAdapter):
        None
...
def demoDebugFields():
    ...
    logger.debug(
        "instance1 pformat output: %s"
        % pprint.PrettyPrinter().pformat(instance1)
    )
