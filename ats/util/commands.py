...
EFFORT_ONLY_OUTPUT_STREAM = (
    "This empty output stream is the output of an EffortOnly command."
)
...
class CommandRunner(object):
    ...
    def issueFirstCommand(self, commandList, argList):
        ...
        message = "Execution of command failed.  None of %s found." % str(
            commandList
        )
        raise UsageError(message)
...
def demo_command():
    ...
    def demoAtLevel(level):
        ...
        for effort_only in (True, False):
            ...
            logger.info(
                "Running command with effort_only=%s:\n" % effort_only
                + "-" * 80
            )
