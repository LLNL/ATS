...
def getStateFromFile(atsrFile):
    ...
    if os.path.exists(atsrFile):
        ...
        exec(
            compile(
                open("src/ats/version.py", "rb").read(),
                "src/ats/version.py",
                "exec",
            )
        )
        ...
        if "logDirectory" in d:
            state["logDirectory"] = d[
                "logDirectory"
            ]  # We want access to the run's logs, but they're not stored as part of the test object
        if "atsMachineName" in d:
            state["atsMachineName"] = d[
                "machineName"
            ]  # Not sure how useful this will be, but it's available.
    else:
...
def getTestStatusLists(testlist):
    ...
    running = [
        test for test in testlist if (test.status is RUNNING)
    ]  # Shouldn't be any in this state because atsr.py is written after execution is done...but that may change so it's here.
...
def atsrToJUnit(atsrFile=None, junitOut=None, build=True):
    ...
    (
        ...
        invalid,
        running,
    ) = getTestStatusLists(state.testlist)
    with open(junitOut, "w") as outf:
        ...
        # NOTE: "writePassedBuild()" not found in current code or history.
...
def writeFailedCodeTestCase(f, test, err_path):
    ...
    msg = ""
    err_files = glob.glob(
        os.path.join(err_path, "*" + str(test.serialNumber) + "*.log.err")
    )
    if len(err_files) > 0:
    ...
    log_files = glob.glob(
        os.path.join(err_path, "*" + str(test.serialNumber) + "*.log")
    )
    if len(log_files) > 0:
...
def writeSkippedTestCase(f, test):
    ...
    testname = cleanTestCaseName(test)
    f.write(
        '    <testcase status="skipped" classname="%s" name="%s"/>\n'
        % (cname, testname)
    )
...
def writeOtherStatusTestCase(f, test):
    ...
    f.write(
        ...
        % (elapsedTime(test), cname, test.name)
    )
    f.write(
        '      <failure type="%s"> Other failure </failure>\n' % (test.status)
    )
    f.write("    </testcase>\n")
...
def writeStatusTestCase(f, test, status):
    ...
    f.write(
        '      <failure type="%s"> ATS set test case to %s.  See ats.log for details. </failure>\n'
        % (test.status, status)
    )
    f.write("    </testcase>\n")
