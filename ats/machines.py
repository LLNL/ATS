...
class MachineCore(object):
    ...
    def getStatus(self, test):
        ...
        if test.child.returncode is None:  # still running, but too long?
                ...
                for line in lines:
                    if slurm_error == False:
                        if (
                            ...
                            print(
                                "ATS Halting test %s. Detected slurm launch failure : %s "
                                % (test.name, line)
                            )
                            slurm_error = True
                        elif "srun: error: Unable to create job step" in line:
                            print(
                                "ATS Halting test %s. Detected slurm error : %s "
                                % (test.name, line)
                            )
                            slurm_error = True
                        elif (
                            "Error opening remote shared memory object in shm_open"
                            in line
                        ):
                            print(
                                "ATS Halting test %s. Detected MPI shared memory failure : %s "
                                % (test.name, line)
                            )
                            slurm_error = True
                        elif (
                            "PSM could not set up shared memory segment" in line
                        ):
                            print(
                                "ATS Halting test %s. Detected MPI shared memory failure : %s "
                                % (test.name, line)
                            ...
                            print(
                                "ATS Halting test %s. Detected MPI Error : %s "
                                % (test.name, line)
                            )
                            slurm_error = True
                        elif "Bus error)" in line:
                            print(
                                "ATS Halting test %s. Detected Bus Error (perhaps MPI related) : %s "
                                % (test.name, line)
                            )
                            slurm_error = True
            ...
            if test.statusCode == 0:  # process is done
                ...
                for line in lines:
                    if lsf_error == False:
                        if "Terminated while pending" in line:
                            print(
                                "ATS Detected LSF Job Start Error %s.  Detected LSF launch failure : %s "
                                % (test.name, line)
                            )
                            lsf_error = True
                    ...
                    for line in lines:
                        if lsf_error == False:
                            if "Error: Locate pipe file" in line:
                                ...
                                print(
                                    "ATS Detected LSF Job Scheduler Error %s.  : %s "
                                    % (test.name, line)
                                )
                                lsf_error = True
                            elif "Error initializing RM" in line:
                ...
                if lsf_error:
                    print(
                        "ATS LSF Development: LSFE Detected statusCode is %d "
                        % test.statusCode
                    )
                    test.statusCode = 2
                    test.setEndDateTime()
    ...
    def testEnded(self, test, status):
        ...
        if MachineCore.debugClass:
            print(
                "DEBUG MachineCore.testEnded invoked cwd= %s " % (os.getcwd())
            )
        ...
        if not (globalPostrunScript == "unset"):
            ...
            if os.path.exists(globalPostrunScript):
                self._executePreOrPostRunScript(
                    globalPostrunScript,
                    test,
                    verbose,
                    globalPostrunScript_outname,
                )
            else:
                log(
                    "ERROR: globalPostrunScript %s not found"
                    % (globalPostrunScript),
                    echo=True,
                )
                sys.exit(-1)
        ...
        if MachineCore.debugClass or MachineCore.canRunNow_debugClass:
            print(
                "DEBUG MachineCore.testEnded decreased self.numberTestsRunning by 1 to %d "
                % self.numberTestsRunning
            )
        ...
        if test.numNodesToUse > 0:
            ...
            if MachineCore.debugClass or MachineCore.canRunNow_debugClass:
                print(
                    "DEBUG MachineCore.testEnded decreased self.numberNodesExclusivelyUsed by %d to %d (max is %d)"
                    % (
                        test.numNodesToUse,
    ...
    def launch(self, test):
        ...
        serial = test.options.get(
            "serial", False
        )  # support serial=True on a per-test basis for backwards compatability for a while
        if nosrun == True or serial == True:
            ...
            if test.commandList == None:
                log(
                    "ATS def launch returning false, commandList is None",
                    echo=True,
                )
                return False
        ...
        if configuration.options.ompNumThreads > 0:
            ...
            if configuration.options.verbose:
                print(
                    "ATS launch setting OMP_NUM_THREADS %d as user specified --ompNumThreads=%d"
                    % (
            ...
            if omp_num_threads > 0:
                if configuration.options.verbose:
                    print(
                        "ATS launch setting OMP_NUM_THREADS %d based on test 'nt'option"
                        % omp_num_threads
                    )
                ...
                # Priority 4 setting, set it to 1 if it is not othewise set
                    ...
                    if configuration.options.verbose:
                        print(
                            "ATS launch setting OMP_NUM_THREADS 1 by default for as it was not specified for the test."
                        )
        ...
        if configuration.SYS_TYPE.startswith("toss"):
            if MachineCore.printExperimentalNotice:
                ...
                print(
                    "ATS Experimental: setting KMP_AFFINITY to %s on Toss"
                    % configuration.options.kmpAffinity
                )
        ...
        if sandbox:
            ...
            if not os.path.isdir(directory):
                ...
                log(
                    "ATS machines.py Creating sandbox directory : %s"
                    % directory,
                    echo=True,
                )
                copytree(
    ...
    def _launch(self, test):
        ...
        if not (globalPrerunScript == "unset"):
            ...
            if os.path.exists(globalPrerunScript):
                self._executePreOrPostRunScript(
                    ...
                    test,
                    verbose,
                    globalPrerunScript_outname,
                )
            else:
                log(
                    "ERROR: globalPrerunScript %s not found"
                    % (globalPrerunScript),
                    echo=True,
                )
                sys.exit(-1)
        ...
        try:
            ...
            if Eadd is None:
                ...
                if MachineCore.debugClass:
                    print(
                        "DEBUG MachineCore._launch env specified =  %s " % Eadd
                    )
                E = os.environ.copy()
                E.update(Eadd)
            ...
            if hasattr(test, "runningWithinSalloc"):
                if configuration.options.sleepBeforeSrun > 0:
                    if MachineCore.printSleepBeforeSrunNotice:
                        ...
                        print(
                            "ATS Info: MachineCore._launch Will sleep %d seconds before each srun "
                            % configuration.options.sleepBeforeSrun
                        )
            ...
            if testStdout == "file":
                ...
                if MachineCore.debugClass:
                    print(
                        "DEBUG MachineCore._launch Invoking Popen 2 %s "
                        % test.commandList
                    )

                if stdin_file is None:
                    test.child = subprocess.Popen(
                        test.commandList, cwd=test.directory, env=E, text=True
                    )
                else:
                    test.child = subprocess.Popen(
                        test.commandList,
                ...
                if MachineCore.debugClass:
                    print(
                        "DEBUG MachineCore._launch Invoking Popen 3 %s "
                        % test.commandList
                    )
            ...
            if MachineCore.debugClass or MachineCore.canRunNow_debugClass:
                print(
                    "DEBUG MachineCore.testEnded increased self.numberTestsRunning by 1 to %d "
                    % self.numberTestsRunning
                )
            ...
            if test.numNodesToUse > 0:
                ...
                if MachineCore.debugClass or MachineCore.canRunNow_debugClass:
                    print(
                        "DEBUG MachineCore._launch__ increased self.numberNodesExclusivelyUsed by %d to %d (max is %d)"
                        % (
                            test.numNodesToUse,
                            self.numberNodesExclusivelyUsed,
                            self.numNodes,
    ...
    def _execute(self, cmd_line, verbose=False, file_name=None, exit=True):
        ...
        if exitCode == 0:
            ...
            if exit:
                log(
                    "%s FATAL RETURN CODE %d Command: %s"
                    % ("ATS", exitCode, cmd_line),
                    echo=True,
                )
                raise SystemExit(1)
    ...
    def _executePreOrPostRunScript(
        self, cmd_line, test, verbose=False, file_name=None, exit=True
    ):
        """
...
class Machine(MachineCore):
    ...
    def __init__(self, name, npMaxH):
        ...
        self.naptime = (
            0.2  # number of seconds to sleep between checks on running tests.
        )
        self.running = []
        self.runOrder = 0
