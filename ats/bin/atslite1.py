...
from ats.util.generic_utils import (
...
def main():
    ...
    for index, arg in enumerate(sys.argv):
        if arg.find("numNodes") >= 0:
            ...
            if val.startswith('"') and val.endswith(
                '"'
            ):  # strip off possible quotes from integer
                val = val[1:-1]
            if val.isdigit():
                numNodes = int(val)
            else:
                ...
                sys.exit(-1)
            print(
                "INFO: atslite1 %s option will use %i nodes" % (arg, numNodes)
            )
            del sys.argv[index]
            break
    ...
    for index, arg in enumerate(sys.argv):
        if arg.find("partition") >= 0:
            ...
            print(
                "INFO: atslite1 will use partition %s" % interactive_partition
            )
    ...
    if os.path.exists(test_ats_file):
        ...
        if os.path.exists(create_test_ats_py):
            ...
            sys.exit(
                "Bummer! Did not find %s or %s"
                % (test_ats_file, create_test_ats_py)
            )
    ...
    if nosub_found is True:
        print(
            "nosub option -- running ATS directly on login or pre-allocated node on %s"
            % sys_type
        )
        ...
        if lsb_batch_jid is None:
            if clean_found is False:
                cmd = (
                    "bsub -q pdebug -nnodes %d -Is -XF -W 120  -core_isolation 2 %s"
                    % (numNodes, cmd)
                )
        ...
        if lsb_batch_jid is None:
            if clean_found is False:
                cmd = (
                    "bsub -nnodes %d -Is -XF -W 120 -G %s -core_isolation 2 %s"
                    % (numNodes, my_bank, cmd)
                )
        ...
        if lsb_batch_jid is None:
            if clean_found is False:
                ...
                cmd = "bsub -x -n %d -Is -XF -W 120 -G %s %s" % (
                    myNumProcs,
                    my_bank,
                    cmd,
                )
        ...
        print(
            "ATS Info: Running script on login node -- each test will have a separate srun allocation"
        )
        cmd = "%s --numNodes=%d" % (cmd, numNodes)

    elif slurm_job_id is None:
        cmd = "salloc -N %d -p %s --exclusive %s" % (
            numNodes,
