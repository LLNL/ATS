...
def main():
    ...
    for index, arg in enumerate(sys.argv):
        if arg.find("partition") >= 0:
            ...
            print(
                "INFO: atslite3 will use partition %s" % interactive_partition
            )
            del sys.argv[index]
    ...
    if nosub_found is True:
        print(
            "nosub option -- running ATS directly without salloc or bsub on %s"
            % sys_type
        )
        ...
        print(
            "Running script on login node -- each test will have a separate srun allocation"
        )
