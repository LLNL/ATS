...
from ats import executables, machines, version
from ats.atsut import abspath, debug
from ats.log import log, terminal
from ats.times import Duration, atsStartTime
...
def add_blueos_only_options(parser):
    parser.add_option(
        "--old_defaults",
        action="store_true",
        dest="blueos_old_defaults",
        help="""Blueos option: Use
    ...
    parser.add_option(
        "--mpibind_executable", default="unset", help="Specify path to mpibind."
    )
    parser.add_option(
        "--lrun",
        action="store_true",
    ...
    parser.add_option(
        ...
        dest="blueos_ngpu",
        type="int",
        help="""Blueos option: Sets of orver-rides
...
def add_more_options(parser):
    ...
    parser.add_option(
        "--oneFailure", action="store_true", help="Stop if a test fails."
    )
    parser.add_option(
        "--reportFreq",
        type="int",
    ...
    parser.add_option(
        "--nosrun", action="store_true", help="Run the code without srun."
    )
    parser.add_option(
        "--salloc",
        action="store_true",
    ...
    parser.add_option(
        "--prerunScript",
        dest="globalPrerunScript",
        default="unset",
        help="Script to run before start of each test.",
    )
    parser.add_option(
        "--postrunScript",
        dest="globalPostrunScript",
        default="unset",
...
def get_machine_factory(
    module_name, machine_class, machine_package="ats.atsMachines"
):
    """
    ...
    """
    machine_module = importlib.import_module(
        f".{module_name}", package=machine_package
    )
...
def get_machine(file_text, file_name, is_batch=False):
    ...
    ats_lines = (
        ats_line
        for ats_line in file_text.splitlines()
    ...
    for line in ats_lines:
        ...
        if machine_name == machine_type:
            ...
            try:
                machine_factory = get_machine_factory(
                    module_name, machine_class
                )
                print(
                    f"from ats.atsMachines.{module_name} "
                    f"import {machine_class} as Machine"
                )
            except ModuleNotFoundError:
                machine_factory = get_machine_factory(
                    module_name, machine_class, machine_package="atsMachines"
                )
                print(
                    f"from atsMachines.{module_name} "
                    f"import {machine_class} as Machine"
                )
...
def init(clas="", adder=None, examiner=None):
    ...
    """
    global log, options, inputFiles, timelimit, machine, batchmachine, defaultExecutable, ATSROOT, cuttime
    ...
    if machine is None:
        terminal(
            "No machine specifications for", SYS_TYPE, "found, using generic."
        )
    ...
    parser = OptionParserWrapper(
        usage=usage, version="%(prog)s " + version.version
    )
    addOptions(parser)
