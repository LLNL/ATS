...
from ats.atsut import CREATED, EXPECTED, PASSED, RUNNING, debug
...
class StandardScheduler(object):
    ...
    def isEligible(self, test):
        ...
        return (
            machine.canRunNow(test)
            and (not self.isBlocked(test))
            and (not self.isWaiting(test))
        )
    ...
    def logStart(self, test, result):
        ...
        if n == 1:
            ...
            if test.srunRelativeNode >= 0:
                ...
                msg = (
                    "%s #%4d (Group %d #%d) %s, %s nn=%i, np=%i, nt=%i, ngpu=%i %s"
                    % (
                        m1,
                        test.serialNumber,
                        test.groupNumber,
    ...
    def periodicReport(self):
        ...
        self.schedule(
            "Remaining:", len(glist), "groups,", len(tleft), "tests.", echo=True
        )
    ...
    def reportObstacles(self, echo=False):
        ...
        if (not machine.numberTestsRunning) or debug():
            ...
            s("------------------------------------------------", echo=echo)
            s(
                "Jobs ready to run but not able to due to wait, block, or cpu",
                echo=echo,
            )
            s(
                "Serial",
            ...
            tc = [
                ...
                for t in chain(*self.groups)
                if t.status is CREATED
                and self.isBlocked(t)
                and (not self.isWaiting(t))
            ]
            if not tc:
                return
            s("------------------------------------------------", echo=echo)
