...
def get_test_lines_generator():
    ...
    test_line = (
        "t%d=test  (executable='./a.out', clas='%s', "
        "label='a.out_%d', np=%d, sandbox=False)\n"
    )
    return (
        test_line % (test_num, arg_, test_num, num_proc)
...
def get_testif_lines_generator():
    ...
    testif_line = (
        "t%d=testif(t%d, executable = my_checker, "
        "clas = t%d.outname, nosrun=True)\n"
    )
    return (
        testif_line % (testif_num, testif_num - 1, testif_num - 1)
